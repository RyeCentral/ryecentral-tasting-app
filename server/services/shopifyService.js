      ...
      products(first: $first, after: $after, query: "product_type:'Rye Whiskey Review' OR tag:'Rye Whiskey Reviews'") {
        edges {
          ...
          node {
            ...
            description
            descriptionHtml
          metafield(namespace: "ryecentral", key: "profile_flavor_notes") {
            references(first: 50) {
              edges {
                node {
                  ... on Metaobject {
...
/**
 * Extract "Top Reported Flavors" pill text from the community section HTML.
 * These are <span class="rc-flavor-tag"> elements in the product description.
 */
function extractTopReportedFlavors(html) {
  if (!html) return [];
  const regex = /<span[^>]*class="[^"]*rc-flavor-tag[^"]*"[^>]*>(.*?)<\/span>/gi;
  const flavors = [];
  let m;
...
function transformToTastingProduct(shopifyProduct) {
  ...
  // Extract individual note keywords for the pill-box game
    ...
    // Extract flavor notes from profile_flavor_notes metafield (priority source for palate pills)
    let metafieldFlavorNotes = [];
    const flavorMeta = shopifyProduct.metafield;
    if (flavorMeta && flavorMeta.references && flavorMeta.references.edges) {
      metafieldFlavorNotes = flavorMeta.references.edges
        .map(edge => {
